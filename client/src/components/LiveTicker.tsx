...
export const LiveTicker: React.FC<LiveTickerProps> = ({ startDate = '2024-03-13T00:00:00' }) => {
  ...
  const StatBox = ({ value, label, color }: { value: number; label: string; color: string }) => (
    <motion.div
      ...
      className="flex flex-col items-center justify-center p-3 sm:p-4 rounded-2xl bg-white/75 backdrop-blur-md border border-rose-100 shadow-xs relative overflow-hidden group min-w-[65px] sm:min-w-[84px]"
    >
      <div className={`text-xl sm:text-3xl font-extrabold tracking-tight ${color}`}>
        {String(value).padStart(2, '0')}
      </div>
      <div className="text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-stone-500 mt-1">
        {label}
      </div>
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-linear-to-r from-rose-100/90 via-pink-50/80 to-amber-100/80 p-6 sm:p-8 border border-rose-200/80 shadow-xl backdrop-blur-xl">
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
        <div>
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-rose-500/10 text-rose-600 text-xs font-bold uppercase tracking-wider mb-2">
          ...
          <p className="text-stone-600 text-sm sm:text-base mt-1 max-w-md">
